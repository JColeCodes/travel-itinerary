...
if (!currentSearch.startDate) {
  var dateInput = $("<div>")
    ...
    timeFrame.find("p").replaceWith(dateInput);
} else {
  timeFrame.find("p").text(currentSearch.dates.start + " to " + currentSearch.dates.end);
}

timeFrame.on("click", "#start-date", function() {
  calendarDiv.removeClass("hidden");
  calendarDiv.addClass("calendar-start");
});
timeFrame.on("blur", "#start-date", function() {
    calendarDiv.addClass("hidden");
    calendarDiv.removeClass("calendar-start");
});
timeFrame.on("click", "#end-date", function() {
  calendarDiv.removeClass("hidden");
  calendarDiv.addClass("calendar-end");
});
timeFrame.on("blur", "#end-date", function() {
    calendarDiv.addClass("hidden");
    calendarDiv.removeClass("calendar-end");
});
calendarDiv.on("mousedown mouseup", function(event) {
